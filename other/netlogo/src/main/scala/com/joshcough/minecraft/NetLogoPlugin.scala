...
class NetLogoPlugin extends CommandsPlugin with ListenersPlugin {
  ...
  val commands = List(
    ...
    Command("setup", "Call the setup proc.")(setup(_)),
    Command("go",    "Call the go proc once.")(go(_)),
    ...
    Command("loop-new",  "Call go until it is finished.", int ~ long.?) {
    ...
    Command("count-entities", "Show the number of entities")(p =>
  ...
  def colors(netLogoColor: Double): MaterialAndData = netLogoColor match {
    ...
    case x if x >= 32  && x <= 38  => DIRT.andData  // choosing not to use brown, but dirt instead
    case x if x >= 42  && x <= 48  => YELLOW.wool
    case x if x >= 52  && x <= 58  => GRASS.andData // choosing not to use green, but grass instead
  ...
  def updateBlockColorMaybe(ws: HeadlessWorkspace, p: Patch, w: World): Unit = {
    ...
    Cube(lowBlock.point, topBlock.point)(w(_)).toStream.foreach(b => colors(p.pcolorDouble).update(b))
...
//   */
//  def updateItemInHand(npc: RemoteEntity, ws: HeadlessWorkspace, t:Turtle): Unit = tryO{
