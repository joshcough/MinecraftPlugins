...
object CommandsPluginAlt {
  ...
  trait CommandsPlugin extends ScalaPlugin with BasicMinecraftParsers {
     ...
     * @return
     */
    def Command[T](commandName: String, desc: String, argsParser: Parser[T])
                  (body: ((Player, T)) => Unit): Command[T] = new Command[T](
      name = commandName,
      description = desc,
     ...
     * @return
     */
    def Command(commandName: String, desc: String)
               (body: Player => Unit): Command[Nothing] = new Command[Nothing](
      name = commandName,
      description = desc,
